...
const DEFAULT_SIDECAR_PATH = "../openclaw-agent-mesh";
...
export function normalizeConfig(raw = {}) {
  ...
  return {
    ...
    localParticipant: optionalString(raw.localParticipant),
...
export function planRuntimeAction(config, action) {
  ...
  if (action?.kind === "mesh_v1_dispatch") {
    return planMeshV1Dispatch(cfg, action, base);
  }
...
function buildEventTaskFollowUp(task, classification, sourceBotId, state = {}) {
  ...
  const nextInstruction = phaseMode
    ...
    : hasRequiredSources && missingSources.length
      ? `Follow-up: reports are still missing from ${missingSources.map((id) => `<@${id}>`).join(", ")}. Each remaining participant should send one ${task.itemLabel}; if the run is complete, declare "${stopPhrase}".`
      : hasRequiredSources
        ? autoComplete
          ? "Follow-up: all required reports are present. Task automatically marked complete."
          : `Follow-up: all required reports are present. If the list is complete, declare "${stopPhrase}".`
        : `Follow-up: continue with the next unreported ${task.itemLabel}. Send one ${task.itemLabel} per message; if the list is complete, declare "${stopPhrase}".`;
...
function buildPhaseFollowUpInstruction(task, stopPhrase, missingSources, phaseAdvance, autoComplete = false) {
  if (missingSources.length) {
    return `Follow-up: only ${missingSources.map((id) => `<@${id}>`).join(", ")} may still respond in this phase. Send one ${task.itemLabel} per message.`;
  }
  if (phaseAdvance?.mentions?.length) {
    return `Follow-up: phase complete. Only ${phaseAdvance.mentions.map((id) => `<@${id}>`).join(", ")} may respond now. Send one ${task.itemLabel} per message; the controller will continue after this phase is complete.`;
  }
  if (autoComplete) {
    return "Follow-up: all required phases are complete. Task automatically marked complete.";
  }
  return `Follow-up: all required phases are complete. If the list is complete, declare "${stopPhrase}".`;
}
  ...
  const label = advisor.classification === "complete"
    ...
    : advisor.classification === "handoff"
      ...
      : advisor.classification === "status"
        ? `please confirm whether this is a ${task.itemLabel} status${advisor.item ? ` for ${advisor.item}` : ""}.`
        : "the controller did not classify this with sufficient confidence; please confirm the next step.";
  return [
    ...
    "Reply naturally; the controller will act only after clear confirmation or new evidence."
  ].join("\n");
}
...
function buildMentionCorrectionMessage(references, sourceParticipant) {
  ...
  return `${mentions} Controller: ${sourceLabel} named ${labels} without a valid Discord tag. The controller is applying the canonical tag so the turn can continue.`;
}
...
export function parseMeshV1Envelope(text = "") {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const match = line.match(/^\s*(cc-mesh|cc-mesh-from|cc-mesh-id|cc-mesh-turn|cc-mesh-final|cc-mesh-seen|hop-limit)\s*:\s*(.*?)\s*$/i);
    if (!match) {
      if (/^\s*(cc-mesh(?:-[a-z]+)?|hop-limit)\b/i.test(line)) errors.push("malformed_mesh_header");
      bodyStart = index;
      break;
    }
    const key = match[1].toLowerCase();
    if (headers.has(key)) errors.push(`duplicate_${key.replaceAll("-", "_")}`);
    else headers.set(key, match[2]);
    bodyStart = index + 1;
  }

  const to = splitMeshList(headers.get("cc-mesh"));
  const from = optionalString(headers.get("cc-mesh-from"));
  const meshId = optionalString(headers.get("cc-mesh-id"));
  const turn = optionalString(headers.get("cc-mesh-turn"));
  const finalHeader = optionalString(headers.get("cc-mesh-final"));
  const final = finalHeader?.toLowerCase() === "true";
  const seen = splitMeshList(headers.get("cc-mesh-seen"));
  const hopLimit = parseMeshInteger(headers.get("hop-limit"));
  ...
  if (finalHeader && !["true", "false"].includes(finalHeader.toLowerCase())) errors.push("invalid_cc_mesh_final");
  if (headers.has("hop-limit") && !Number.isInteger(hopLimit)) errors.push("invalid_hop_limit");
...
export function planMeshV1Dispatch(config = {}, action = {}, inheritedBase) {
  const base = inheritedBase ?? {
    accepted: false,
    ...
    reason: "unclassified",
    sideEffectsAllowed: false,
    auditRequired: config.audit?.enabled !== false
  };
  const localParticipant = optionalString(config.localParticipant ?? config.participant ?? action.localParticipant);
  const envelope = parseMeshV1Envelope(action.text ?? action.messageText ?? action.content ?? action.request?.text ?? "");
  const state = normalizeMeshV1State(action.state ?? action.request?.state);
  const messageId = optionalString(action.messageId ?? action.request?.messageId);

  if (!envelope.valid) {
    return { ...base, reason: "mesh_v1_invalid_envelope", nextAction: "none", envelope };
  }
  if (!localParticipant) {
    return { ...base, reason: "mesh_v1_local_participant_required", nextAction: "none", envelope };
  }
  if (envelope.turn !== localParticipant) {
    return { ...base, reason: "mesh_v1_not_local_turn", nextAction: "none", envelope };
  }
  if (!envelope.to.includes(localParticipant)) {
    return { ...base, reason: "mesh_v1_local_not_addressed", nextAction: "none", envelope };
  }
  if (envelope.seen.includes(localParticipant)) {
    return { ...base, reason: "mesh_v1_loop_guard_seen", nextAction: "none", envelope };
  }
  if (Number.isInteger(envelope.hopLimit) && envelope.hopLimit <= 0) {
    return { ...base, reason: "mesh_v1_hop_limit_exhausted", nextAction: "none", envelope };
  }

  const recordKey = `${envelope.meshId}::${envelope.from}::${localParticipant}`;
  const record = state.records[recordKey] ?? { parts: [], seenFingerprints: [], dispatched: false };
  const fingerprint = messageId ?? `${envelope.from}:${envelope.meshId}:${envelope.final ? "final" : "partial"}:${envelope.body}`;
  if (record.seenFingerprints.includes(fingerprint) || (envelope.final && record.dispatched)) {
    return {
      ...base,
      accepted: true,
      reason: "mesh_v1_duplicate_suppressed",
      nextAction: "none",
      sideEffectsAllowed: false,
      envelope,
  ...
  const nextRecord = {
    parts: envelope.body ? [...record.parts, envelope.body] : [...record.parts],
    seenFingerprints: [...record.seenFingerprints, fingerprint],
    dispatched: envelope.final ? true : record.dispatched
  };
  ...
  if (!envelope.final) {
    return {
      ...base,
      accepted: true,
      reason: "mesh_v1_partial_buffered",
      nextAction: "buffer_only",
      sideEffectsAllowed: false,
      envelope,
      stateTransition
    };
  ...
  return {
    ...base,
    accepted: true,
    reason: "mesh_v1_final_dispatch_ready",
    nextAction: "dispatch_once",
    sideEffectsAllowed: false,
    envelope,
    dispatchText: nextRecord.parts.join("\n"),
    stateTransition
  };
}

function normalizeMeshV1State(value = {}) {
  const records = value?.records && typeof value.records === "object" ? value.records : value?.meshV1?.records;
  const normalizedRecords = {};
  if (records && typeof records === "object") {
    for (const [key, record] of Object.entries(records)) {
      normalizedRecords[key] = {
        parts: Array.isArray(record?.parts) ? record.parts.filter((part) => typeof part === "string") : [],
        seenFingerprints: Array.isArray(record?.seenFingerprints) ? record.seenFingerprints.filter((entry) => typeof entry === "string") : [],
