...
export const RUNNER_PLAN_SCHEMA = "openclaw.agent.runner_plan.v1" as const;

export const RUNNER_PLAN_STATUSES = ["planned_stub_only", "blocked"] as const;
export type RunnerPlanStatus = (typeof RUNNER_PLAN_STATUSES)[number];

export const RUNNER_OPERATION_MODES = ["analysis", "code_edit", "test", "review"] as const;
export type RunnerOperationMode = (typeof RUNNER_OPERATION_MODES)[number];

export const RUNNER_APPROVAL_POLICIES = [
  "not_required_read_only",
  "ask_before_write",
  "ask_before_commit",
  "ask_before_push"
] as const;
export type RunnerApprovalPolicy = (typeof RUNNER_APPROVAL_POLICIES)[number];

export interface RunnerPlanRecord {
  schema: typeof RUNNER_PLAN_SCHEMA;
  id: string;
...
export function validateRunnerPlanRecord(
  input: unknown
): ValidationResult<RunnerPlanRecord> {
  if (!isRecord(input)) {
    return fail([{ path: "$", message: "must be a JSON object" }]);
  }

  const issues: ValidationIssue[] = [];
  const schema = requireEnum(input, "schema", [RUNNER_PLAN_SCHEMA], issues);
  const id = requireString(input, "id", issues);
  const executionJobId = requireString(input, "execution_job_id", issues);
  const status = requireEnum(input, "status", RUNNER_PLAN_STATUSES, issues);
  ...
  const runnerRoles = requiredStringArray(input, "runner_roles", issues);
  const operationMode = requireEnum(
    input,
    "operation_mode",
    RUNNER_OPERATION_MODES,
    issues
  );
  const approvalPolicy = requireEnum(
    input,
    "approval_policy",
    RUNNER_APPROVAL_POLICIES,
    issues
  );
  ...
  const noRealRunnerAdapterCall = requireTrue(input, "no_real_runner_adapter_call", issues);
  ...
  return ok({
    ...
    runner_roles: runnerRoles!,
    operation_mode: operationMode!,
    approval_policy: approvalPolicy!,
    allowed_actions: allowedActions!,
    forbidden_actions: forbiddenActions!,
    no_external_side_effects: noExternalSideEffects!,
    no_real_runner_adapter_call: noRealRunnerAdapterCall!,
