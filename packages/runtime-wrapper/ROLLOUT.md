...
- append-only audit persistence confirmed at `/path/to/runtime/agent-mesh-wrapper/audit.jsonl` for the current gateway working directory;
...
- Agent Alpha is now explicitly allowlisted as a real bridge participant for the corrected orchestration channel;
- split body/footer messages are normalized as one logical bridge turn through `request.messages`;
- live split body/footer smoke using Agent Alpha messages `DISCORD_ID_PLACEHOLDER` + `DISCORD_ID_PLACEHOLDER` returns `bridge_terminal_done`, `nextAction: "stop"`, and `normalizedTurn.splitMessages: true`;
- allowlisted `NEXT` display labels such as `@Agent Beta` are normalized to the configured mention before handoff planning;
...
- live raw-bridge E2E `orch-real-2026-05-11-i` succeeded: Agent Alpha emitted `NEXT: @Agent Beta` with no real Discord mention, the raw gateway listener accepted and normalized it to `<@DISCORD_ID_PLACEHOLDER>`, and the controller terminal send succeeded as Discord message `DISCORD_ID_PLACEHOLDER`;
...
- bounded run-scoped live smoke passed after restart: `RunId: agent-os-smoke-20260516T140016Z` flowed through Agent Alpha -> Agent Gamma -> Agent Beta in channel `DISCORD_ID_PLACEHOLDER`; controller advanced phases and auto-completed with final send `DISCORD_ID_PLACEHOLDER`.

## Fast safe rollout sequence

1. [x] Add/expose the wrapper in observe mode only.
2. [x] Restart/reload only after the operator explicitly approves the config change.
...
6. [x] Move to `mode: "plan"` after the operator approved the required reload/config step.
...
10. [x] Restart/reload only after the operator explicitly approves activation of the listener patch.
11. [x] Implement address-book/correction-bridge planner and runtime patch for untagged agent-name mentions.
12. [x] Restart/reload after the operator approved activation of the correction bridge.
13. [x] Run controlled live addressing test: plain name and controller correction.
14. [x] Diagnose realistic one-container-at-a-time test latency; root cause was manual Agent Beta polling/session overhead, not Agent Alpha response time.
...
20. [x] Run controlled live event-driven multi-agent test with a fresh explicit `RunId` and no manual Agent Beta polling.
...
2. reload/restart OpenClaw only with the operator approval;
...
4. preserve the runtime audit log for review (`/path/to/runtime/agent-mesh-wrapper/audit.jsonl` in the current gateway process);
...
- require explicit channel/thread + bot identity allowlists before accepting bot-authored handoffs; current live participant allowlist includes Agent Alpha and Agent Beta for the corrected orchestration test path;
