...
- allowlisted `NEXT` display labels such as `@Karan S'Jet` are normalized to the configured mention before handoff planning;
...
- live raw-bridge E2E `orch-real-2026-05-11-i` succeeded: YehonalBot emitted `NEXT: @Karan S'Jet` with no real Discord mention, the raw gateway listener accepted and normalized it to `<@DISCORD_ID_PLACEHOLDER>`, and the controller terminal send succeeded as Discord message `DISCORD_ID_PLACEHOLDER`;
...
- bounded run-scoped live smoke passed after restart: `RunId: agent-os-smoke-20260516T140016Z` flowed through YehonalBot -> Odino -> Karan - NestDev in channel `DISCORD_ID_PLACEHOLDER`; controller advanced phases and auto-completed with final send `DISCORD_ID_PLACEHOLDER`.
...
14. [x] Diagnose realistic one-container-at-a-time test latency; root cause was manual Karan polling/session overhead, not YehonalBot response time.
...
20. [x] Run controlled live event-driven multi-agent test with a fresh explicit `RunId` and no manual Karan polling.
...
- require explicit channel/thread + bot identity allowlists before accepting bot-authored handoffs; current live participant allowlist includes YehonalBot and Karan for the corrected orchestration test path;
