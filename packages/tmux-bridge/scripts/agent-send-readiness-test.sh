#!/usr/bin/env bash
# Regression test: do not paste linked turns into unavailable agent sessions.
...
BUSY_AGENT="busy-test-$$"
BUSY_TARGET="mesh-busy-test-$$"
BUSY_CONF="$AGENTS_DIR/${BUSY_AGENT}.conf"
...
cleanup() {
    ...
    rm -f "$BUSY_CONF" "$REPAINT_CONF" "$COMPOSER_CONF" "$GHOST_CONF"
    exit "$status"
}
...
) || fail "Claude readiness patterns do not recognize current UI states"
...
tmux -L "$MESH_TMUX_SOCKET" new-session -d -s "$BUSY_TARGET" \
    "bash -c 'while true; do echo Working; sleep 0.1; done'"
set +e
output="$("$SEND_BIN" --quiet --agent "$BUSY_AGENT" "$BUSY_TARGET" "$PROMPT" 2 2>&1)"
status=$?
set -e
[[ "$status" -eq 6 ]] || fail "expected busy exit 6, got $status ($output)"
[[ "$output" == *"prompt was not pasted"* ]] || fail "busy result omitted no-paste guarantee"
pane="$(tmux -L "$MESH_TMUX_SOCKET" capture-pane -t "$BUSY_TARGET" -p)"
...
set -e
[[ "$status" -eq 6 ]] \
    || fail "idle repaint falsely confirmed submission: expected 6, got $status ($output)"
[[ "$output" == *"NOT_SUBMITTED"* ]] || fail "unconfirmed submit did not identify its blocked state"
...
status=$?
set -e
[[ "$status" -eq 6 ]] || fail "expected occupied-composer exit 6, got $status ($output)"
[[ "$output" == *"COMPOSER_OCCUPIED"* ]] || fail "occupied composer was not identified"
...
set -e
[[ "$status" -eq 6 ]] \
    || fail "ghost suggestion did not reach submit confirmation: expected 6, got $status ($output)"
[[ "$output" != *"COMPOSER_OCCUPIED"* ]] || fail "ghost suggestion was treated as occupied input"

echo "PASS: unavailable agents are blocked before paste and ghost suggestions pass"
