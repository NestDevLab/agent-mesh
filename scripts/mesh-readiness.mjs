...
check("public-tree privacy sanity", () => { const deny = [snowflakeIdPattern, /\/home\/administrator\/env\/workspace\/example-business\//, /(api[_-]?key|secret|password|token)\s*[:=]\s*["'][^"']{6,}["']/i]; const privateIdentifiers = ["ti" + "rrenia", "od" + "ino"]; const skipDirs = new Set([".git", "node_modules", "dist"]); const skipPaths = new Set([".syncwheel/ledger"]); const hits = []; function walk(dir) { if (skipPaths.has(path.relative(root, dir))) return; for (const entry of readdirSync(dir)) { if (skipDirs.has(entry)) continue; const full = path.join(dir, entry); const stat = statSync(full); if (stat.isDirectory()) walk(full); else if (stat.isFile()) { const text = readFileSync(full, "utf8"); for (const pattern of deny) if (pattern.test(text)) hits.push(path.relative(root, full)); if (privateIdentifiers.some((identifier) => text.toLowerCase().includes(identifier))) hits.push(path.relative(root, full)); } } } walk(root); assert(hits.length === 0, `privacy scan hits: ${[...new Set(hits)].join(", ")}`); });
