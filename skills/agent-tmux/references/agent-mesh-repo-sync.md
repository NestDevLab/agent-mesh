...
Use this when work on the tmux/Agent Mesh skill changes local installed skills, Hermes lookup behavior, or the `agent-mesh` repository.

## Durable lesson

Do not stop at fixing the installed Hermes skill or the runtime copy. If `agent-mesh` is the source of truth for `agent-tmux-bridge`, update the repository, verify it, push it, then sync the installed skill from that source.
...
1. Identify the canonical repo and installed skill paths.
   ...
   - Installed skill: profile/global Hermes skills tree that exposes `agent-tmux-bridge`.
