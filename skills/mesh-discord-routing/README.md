...
mesh-hydrate --to odino,tirrenia --body "Answer: yes."
```
...
```bash
mesh-session start --session-id demo-channel --channel-id channel-id --participants odino,tirrenia --policy round_robin --active odino
mesh-session next --state state/demo-channel.json --from odino
mesh-session next --state state/demo-channel.json --from tirrenia --selected odino
```
