...
mesh-hydrate --to odino --to tirrenia --body "Answer: yes."
```
...
```json
{
  ...
  "facilitator": "main",
  "guessers": ["tirrenia"],
  "activeGuesser": "tirrenia",
  "guesserQueue": ["tirrenia"],
  "target": "secret word",
...
2. For multiple guessers, create mesh turn state:
   ...
   ```bash
   mesh-session start --session-id taboo-YYYYMMDDTHHMMSSZ-channel-id --channel-id channel-id --participants odino,tirrenia --policy round_robin --active odino --skill taboo-advanced
   ```
...
```text
Taboo Advanced: Tirrenia guesses. Limit: 6 turns. Start with one yes/no question; guess only when you have enough clues.
```
